...
新しいイメージの内容がどちらの環境にも反映されない状態になった。
pushした時点が「このタグを配布する」と決めた瞬間なので、そこで揃えてしまう。

単独で実行することもできる。冪等なので、状態を確認したいだけの場合にも使える。
...
**タグは手順4の書き換えを含むコミットに対して打つこと。**
先にタグを打つと、そのタグが指すコミットの`FROM`は古いバージョンのままになり、
「そのバージョンの配布構成」を後から再現できなくなる。
