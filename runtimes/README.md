...
- 軽量ランタイムイメージ
- **位置づけ**: 別プロジェクトへCI/CDのランタイムとして提供するための補助的なツール群。
  正式な配布環境は`envs/`配下（devcontainer / denv-cc-remote）であり、こことは扱いが異なる。
  こちらはドキュメントを整備したサポート対象の成果物ではない。
- `latest`タグは付与しない。ビルド時期で中身が変わるため、指す実体が不定になる。
