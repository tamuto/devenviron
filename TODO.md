...
- [x] **3.4 必要要件の洗い出し（主要部分）**
  ...
  - **Serena MCP の登録方法【対応済み・実害あり】**
    ...
    - 対応: entrypoint スクリプトを追加し、
      コンテナ起動のたびに `claude mcp get serena` で登録状態を確認して
      不足していれば登録する方式へ変更した。
      イメージ更新時も既存環境へ反映される自己修復型になっている。
    - あわせて、bind mount 直後の空ファイル（JSON として不正）を
      entrypoint で `{}` に正規化するようにした。
  - 残っている検討事項:
